...
async def changeStates(uri, state):
    async with websockets.connect(uri) as websocket:
        for k in state:
            if k != 'type':
                # if ((k == '2' or k == '14' or k == '4' or k == '17') and state[k] == False) or (k != '2' and k != '14' and k != '4' and k != '17' and state[k] == True):
...
async def startAlarm(uri):
    async with websockets.connect(uri) as websocket:
        ...
        alarms = ['file:///media/pi/KINGSTON/muzyka/lp3/LP3_1983/02%20Kocham%20Cie%2C%20kochanie%20moje.wma']
...
asyncio.get_event_loop().run_until_complete(
    startAlarm('ws://192.168.1.12:6680/mopidy/ws'))
