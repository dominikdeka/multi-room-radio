...
async def turnOnTurnOff(uri):
    async with websockets.connect(uri) as websocket:
        ...
        for k in data:
            if k != 'type':
                if ((k == '2' or k == '4' or k == '14' or k == '18') and data[k] == False) or (k != '2' and k != '4' and k != '14' and k != '18' and data[k] == True):
...
async def startWakeup(uri, prefix):
    async with websockets.connect(uri) as websocket:
        ...
        await websocket.send(json.dumps({"method":"core.tracklist.add","params":{"uri":"local:track:muzyka%20dla%20dzieci/budzenie/mix3.mp3"},"jsonrpc":"2.0","id":87}, indent='\t'))
